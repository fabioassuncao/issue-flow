...
/**
 * PORT of `frontend/src/lib/worktree-conversation.test.ts` @ d8c9d5f — 15
 * cases, unchanged. The fixtures stay in English: they are agent-conversation
 * payloads, not interface strings.
 */
...
describe('worktree conversation helpers', () => {
  it('adds optimistic user messages when a turn starts', () => {
    expect(
      markConversationTurnStarted(makeConversation(), 'turn-2', 'Ship it')?.messages.at(-1),
    ).toEqual({
  ...
  it('captures progress when the latest message grows', () => {
    ...
    expect(buildConversationProgressSignature(started)).not.toBe(
      buildConversationProgressSignature(updated),
    );
  });
  ...
  it('keeps optimistic user messages when only the same server text arrives for another turn', () => {
    ...
    expect(merged.messages.some((message) => message.id === 'pending-user:client-turn-2')).toBe(
      true,
    );
