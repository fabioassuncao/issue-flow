...
/**
 * PORT of `frontend/src/lib/MobileChatSurface.test.ts` @ d8c9d5f — 11 cases.
 * These are the guards on the single long-lived stream: one connection across
 * turns, out-of-order revisions dropped, and the polling fallback settling.
 */
...
describe('MobileChatSurface', () => {
  ...
  it('streams Claude conversations without polling history after sending', async () => {
    ...
    await screen.findByText('Nenhuma mensagem ainda. Envie o primeiro prompt para começar.');
    ...
    await waitFor(() => {
      expect(sendWorktreeConversationMessage).toHaveBeenCalledWith('feature/mobile-chat', {
        text: 'Ship it',
      });
    });
    await waitFor(() => {
  ...
  it('reuses one Claude stream across turns instead of reconnecting', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('Mensagem'), { target: { value: 'first' } });
    await fireEvent.click(screen.getByRole('button', { name: 'Enviar' }));
    ...
    await fireEvent.input(screen.getByLabelText('Mensagem'), { target: { value: 'second' } });
    await fireEvent.click(screen.getByRole('button', { name: 'Enviar' }));
    await waitFor(() => {
  ...
  it('renders Codex stream events in their explicit order', async () => {
    ...
    expect(text.indexOf('First assistant')).toBeLessThan(text.indexOf('Concluído shell'));
    expect(text.indexOf('Concluído shell')).toBeLessThan(text.indexOf('Second assistant'));
  });
