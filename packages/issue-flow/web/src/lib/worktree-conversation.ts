...
/**
 * PORT of `frontend/src/lib/worktree-conversation.ts` @ d8c9d5f (201 lines),
 * unchanged in behaviour.
 *
 * This is the reducer for the structured chat: ordered messages, streamed
 * deltas, upserts, and the optimistic user message. Three details here exist
 * for a specific failure and none of them is decoration:
 *
 * - **`isOptimisticUserMessage` + `isServerUserForPendingTurn`.** The user's
 *   own message is rendered before the server has acknowledged it. When the
 *   server's copy arrives it has a different id, so matching on id alone would
 *   show the message twice. Matching the pending one by `turnId` replaces it
 *   instead. This is the same class of problem the canonical Claude parser
 *   solves with its `${messageId}:${blockIndex}` block identity (§45.2-A).
 * - **`mergeConversationSnapshot` preserving an unacknowledged optimistic
 *   message.** A history refetch that landed before the server recorded the
 *   send would otherwise erase what the user just typed.
 * - **`buildConversationProgressSignature`.** Cheap change detection for the
 *   polling path, so a provider without live events still settles instead of
 *   polling forever.
 */
...
export function applyConversationMessageDelta(
  ...
  const existingMessage =
    existingIndex === -1 ? null : (conversation.messages[existingIndex] ?? null);
...
export function applyConversationMessageUpsert(
  ...
  return {
    ...
    activeTurnId:
      event.message.status === 'inProgress' ? event.message.turnId : conversation.activeTurnId,
    messages: orderConversationMessages(messages),
  };
}
